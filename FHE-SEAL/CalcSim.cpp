...
int main(int argc, char* argv[])
{
	...
	*/
	...
	Ciphertext Trapdoor;
	Trapdoor.unsafe_load(cin);
//	vector<Ciphertext> Index(num);
//	for(int i=0;i<num;i++)Index[i].unsafe_load(cin);
	/*
	TRAPDOOR EXPANSION
	*/
//	vector<Ciphertext> masks_processed(num);
	Ciphertext result;
	...
	for(int i=0;i<num;i++){
		...
		evaluator.multiply_plain(Trapdoor,masks[i],mask);
		Ciphertext temp;
