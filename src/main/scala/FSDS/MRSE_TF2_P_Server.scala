...
class MRSE_TF2_P_Server(settings: Settings) extends MRSE_TF2_Server(settings) {
  ...
  override def SimilaritySearch(query: Query): Result = {
    ...
    for (i <- 0 until settings.chunkNum) {
      ...
      threads(i)=new Thread(){
        override def run(): Unit = {
          ...
          val partRes=(0 until ChunkSize).map(u=>(u+ID*ChunkSize,indexPart(u).dotProduct(query.get().
            asInstanceOf[ArrayRealVector]))).toArray
          val sortedRes=Util.Functions.Insert(partRes,settings.K)
    ...
    val cummRes=allRres.reduce((a,b)=>Util.Functions.Insert(a,b,settings.K))
